...
def cfac_past_path(exp):
    return rf'./data/final_exp/counterfactual/GWL-1.5/{exp}/wa_max.nc'
...
def plot_cfac(ax, path_func, stat):
    ...
    for exp in model_list_ori:
        ...
        tp_cfac = ds_cfac['wa'].mean(dim='time')
    ...
    ax.pcolormesh(tp_cfac.xlon, tp_cfac.xlat, ens_stat.values, cmap=cmap, shading="auto",
            transform=ccrs.PlateCarree(), zorder=1, vmin=0, vmax=pr_max)
    
if __name__ == '__main__':
    # Parameters
    outfile = rf'./figs/compare/finals/map_updraft_mean.png'
    stat = 'mean'
    ...
    # Plot for factual
    path_fac = r'./data/final_exp/factual/wa_max.nc'
    ds_fac = read_data(path_fac, time1, time2, rename=True)
    tp_fac = ds_fac['wa'].mean(dim='time')

    pcm = axs[1].pcolormesh(tp_fac.xlon, tp_fac.xlat, tp_fac.values, cmap=cmap, shading="auto",
                transform=ccrs.PlateCarree(), zorder=1, vmin=0, vmax=pr_max)
