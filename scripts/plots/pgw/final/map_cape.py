...
def cfac_past_path(exp):
    return rf'./data/final_exp/counterfactual/GWL-1.5/{exp}/cape_SRF.nc'
...
def plot_cfac(ax, path_func, stat):
    ...
    for exp in model_list_ori:
        ...
        tp_cfac = ds_cfac['cape'].mean(dim='time')
...
if __name__ == '__main__':
    # Parameters
    outfile = rf'./figs/compare/finals/map_cape.png'
    stat = 'mean'
    ...
    # Plot for factual
    path_fac = r'./data/final_exp/factual/cape_SRF.nc'
    ds_fac = read_data(path_fac, time1, time2)
    tp_fac = ds_fac['cape'].mean(dim="time")
