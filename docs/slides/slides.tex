...
\begin{frame}
    ...
    \begin{block}{Research objectives}
        \begin{enumerate}
            \item \textbf{Factual simulation:} Simulate the 2025 extreme rainfall and cyclone dynamics associated with TC Senyar in Sumatra with high resolution RegCM5 model under present climate.
            \item \textbf{Historical attribution:} Simulate a counterfactual model where the warming signal is removed (-1.5$^\circ$) from the boundary conditions to represent a pre-industrial climate.
...
\begin{frame}
    ...
    \begin{block}{Track definition}
        At each time step, the track is defined at a coordinate with the lowest sea level pressure \citep{patricola2018anthropogenic}.
    \end{block}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Moisture transport is quantified by its vertical integration between the surface and 700 hPa, as moisture transport associated with TCs is primarily concentrated in the lower troposphere \citep{li2024western}.
    \end{itemize}
\end{frame}
...
\begin{frame}
    \begin{itemize}
            \item Convection-permitting RegCM5 model improves the representation of TC Senyar track, intensity, and accumulated precipitation than ERA5.
    \end{itemize}
\end{frame}
