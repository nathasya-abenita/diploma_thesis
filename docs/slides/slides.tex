...
\begin{frame}
    ...
    \begin{block}{Storyline attribution study}
        \begin{itemize}
            ...
            \item Pseudo-global warming (PGW) method is one of the approach.
        \end{itemize}
    \end{block}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{block}{Research objectives}
        \begin{enumerate}
            \item \textbf{Factual simulation:} Simulate the 2025 extreme rainfall and cyclone dynamics associated with TC Senyar in Sumatra with high resolution RegCM5 model under \textbf{present climate}.
            \item \textbf{Historical attribution:} Simulate a counterfactual model where the warming signal is removed (\textbf{-1.5$^\circ$}) from the boundary conditions to represent a \textbf{pre-industrial} climate.
            \item \textbf{Future projection:} Simulate a \textbf{future} projection by adding a \textbf{+1.5$^\circ$} warming signal to the boundary conditions to assess the hazard's evolution in a warmer world.
...
\begin{frame}
    ...
    \begin{block}{Track definition}
        At each time step, the track is defined at a coordinate with the lowest sea level pressure, also used by \citet{patricola2018anthropogenic}.
    \end{block}
    \end{columns}
\end{frame}
...
\begin{frame}
    ...
    \begin{block}{Steps in applying PGW}
        \begin{enumerate}
            \item Prepare monthly mean of variable $X$ from different periods of CMIP6 data.
            \item Compute perturbation. For example, "cooling" perturbation from EC-Earth3-Veg at month $m$ is 
...
% \begin{frame}
%     \begin{itemize}
%         \item We focus on observed peak days, between 25 November 2025 01:00 UTC and 26 November 2025 23:00 UTC. \pause
%         \item Data is sorted and presented as exceedance probability plot (equivalent to $1-\text{PCTL}/100$ where $\text{PCTL}$ is percentile). Relative change from a distribution to another distribution is assessed on a given percentile. \pause
...
%         \end{equation} \pause
%         \item The Clausius-Clapeyron scaling analysis is done to study the response of the variables to global warming level change. We divide $X_{\text{increase}}$ in Eq. \eqref{eq:increase-1} and \eqref{eq:increase-2} with 1.5$^\circ$ which corresponds to the experiment design.
%     \end{itemize}
% \end{frame}
...
\subsection{Factual Simulation of Tropical Cyclone (TC) Senyar with RegCM5}
...
\begin{frame}
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item  ERA5 is able to represent the big pattern of the event evolution with weaker intensities \citep{dulac2024assessing}.
        \item The presented factual simulation is the best experiment out of different trials on initialization time, domain, and physical schemes.
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item In future climate, peak wind intensity consistently increases. This is consistent with previous attribution study on TCs \citep{patricola2018anthropogenic}.
    \end{itemize}
    }
\end{frame}

\subsection{Conceptual Framework of Analyzed Variables}

\begin{frame}
    \vspace{-0.5cm}
    \centering
    {
    \includegraphics[width=0.7\textwidth]{../figs/intro_schematic.png} 
    \vspace{-0.5cm}\newline
    \vspace{-0.1cm}
    {
    \scriptsize 
    Changes in extreme precipitation processes
    }
    }
    \vspace{0.5cm}
    \begin{itemize}
        \item Short-duration extremes can intensify at Clausius-Clapeyron (CC) rates or exceeding it due to \textbf{thermodynamic and dynamical feedbacks} \citep{fowler2021}.
        \item Moisture transport is quantified by integrated vapor transport (IVT) between surface and 700 hPa, as moisture transport associated with TCs is primarily concentrated in the lower troposphere \citep{li2024western}.
    \end{itemize}
\end{frame}

\subsection{Anthropogenic influences on \textbf{precipitation} associated with TC Senyar}

\begin{frame}
    \centering

    % \begin{textblock*}{4cm}(9cm,1cm)
    % \includegraphics[width=0.2\textwidth]{../../figs/event_view.png}
    ...
    \vspace{0.5cm}
    \begin{itemize}
        \item Factual simulation with RegCM5 improves spatial distribution of accumulated precipitation compared to ERA5.
    \end{itemize}
    }
\end{frame}

\begin{frame}
    % \centering{\textbf{Accumulated Precipitation and Hourly Intensity}}
    \vspace{-0.5cm}
    \begin{figure}[ht]
        \centering
...
\subsection{Anthropogenic influences on \textbf{physical drivers} controlling extreme rainfall}
...
\begin{frame}
    \vspace{-0.5cm}
    {\alert{\small\textbf{Integrated Vapor Transport (IVT)}}}
    \begin{figure}[ht]
        \centering
...
\begin{frame}
    \vspace{-0.5cm}
    {\alert{\small\textbf{Convective Available Potential Energy (CAPE)}}}
    \begin{figure}[ht]
        \centering
        \begin{subfigure}{0.75\textwidth}
            \centering
...
\begin{frame}
    \vspace{-0.5cm}
    {\alert{\small\textbf{Latent Heating Rate}}}
    \begin{figure}[ht]
        \centering
        \begin{subfigure}{0.75\textwidth}
            \centering
...
\begin{frame}
    \vspace{-0.5cm}
    {\alert{\small\textbf{Updraft (Maximum Vertical Velocity)}}}
    \begin{figure}[ht]
    \begin{subfigure}{0.75\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../../figs/compare/finals/map_wa_med.png}
        % \caption{}
    \end{subfigure}
    % \vspace{1em} % optional spacing
    \begin{subfigure}{0.75\textwidth}
    ...
    \caption*{\scriptsize (Top) Spatial map of time-maximum of maximum vertical velocity during peak days. Maximum vertical velocity is obtained by taking vertical maxima over each time step. Ensemble median is used for counterfactuals. (Bottom) Distribution change is based on pooled six-hourly values from whole shown domain.}
...
\begin{frame}
    \begin{itemize}
            \item Convection-permitting RegCM5 model improves the representation of TC Senyar track, intensity, and accumulated precipitation compared to ERA5.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    {
    \scriptsize 
    Values indicate the relative change for the historical attribution (past-to-present) and future projection (present-to-future) scenarios. Blue boxes highlight localized impacts within the Aceh Province, while gray boxes show regional changes.
    }
    }
    \newline
    \begin{itemize}
